...
class MainViewController: UIViewController {
    ...
    typealias creatorInitPrototype = @convention(c) ()->NSObject
    ...
    @IBAction func loadAction(_ sender: UIBarButtonItem) {
        if handle == nil{
            handle = dlopen("/Users/admin/Desktop/Projects/OOP-6-Dynamic/DynamicFramework/Trapezium.framework/Trapezium" , RTLD_LAZY)
            if handle != nil {
                if let creatorSym = dlsym(handle, "trapezium_creator_init") {
                    let creatorFunc = unsafeBitCast(creatorSym, to: creatorInitPrototype.self)
                    let creator = creatorFunc()  as! ShapeCreator
                    ShapeOptions.addShapeCreator(creator: creator)
                }
            }
        }
    }
